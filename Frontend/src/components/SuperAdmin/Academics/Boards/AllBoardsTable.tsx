...
const AllBoardsTable: React.FC = () => {
  const [search, setSearch] = useState<string>('');
  const [tableData, setTableData] = useState<BoardData[]>(initialData);
  const [rowToDelete, setRowToDelete] = useState<BoardData | null>(null);
  ...
  const columns = [
    { name: 'ID', selector: (row: BoardData) => row.id, width: '60px', center: true },
    {
      name: 'Chapter Number',
      selector: (row: BoardData) => row.chapternumber,
      sortable: true,
      cell: (row: BoardData) => <div className='chapter_number'>{row.chapternumber}</div>,
    },
    { name: 'Chapter Name', selector: (row: BoardData) => row.name, sortable: true },
    { name: 'Topics', selector: (row: BoardData) => row.topics },
    {
      name: 'Subject',
      selector: (row: BoardData) => row.subject,
      center: true,
      cell: (row: BoardData) => <div className='subject_name_tag'>{row.subject}</div>,
    },
    { name: 'Board', selector: (row: BoardData) => row.board, center: true },
    { name: 'Class', selector: (row: BoardData) => row.class, center: true },
    {
      name: 'Actions',
      cell: (row: BoardData) => (
  ...
  const handleAddClick = () => navigate('/academics/boards/create-board');
  const handleView = (row?: BoardData) => navigate('/academics/boards/view-board');
  const handleEdit = (row?: BoardData) => navigate('/academics/boards/edit-board');
  const handleDelete = (row: BoardData) => {
    setRowToDelete(row);
    setIsModalOpen(true);
  };
