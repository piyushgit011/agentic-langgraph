...
export default function AllClassesTable() {
  ...
  const [tableData, setTableData] = useState<ClassData[]>(initialData);
  const [rowToDelete, setRowToDelete] = useState<ClassData | null>(null);
  ...
  const columns = [
    { name: 'ID', selector: (row: ClassData) => row.id, width: '60px', center: true },
    {
      name: 'Chapter Number',
      selector: (row: ClassData) => row.chapternumber,
      sortable: true,
      cell: (row: ClassData) => <div className='chapter_number'>{row.chapternumber}</div>,
    },
    { name: 'Chapter Name', selector: (row: ClassData) => row.name, sortable: true },
    { name: 'Topics', selector: (row: ClassData) => row.topics },
    {
      name: 'Subject',
      selector: (row: ClassData) => row.subject,
      center: true,
      cell: (row: ClassData) => <div className='subject_name_tag'>{row.subject}</div>,
    },
    { name: 'Board', selector: (row: ClassData) => row.board, center: true },
    { name: 'Class', selector: (row: ClassData) => row.class, center: true },
    {
      name: 'Actions',
      cell: (row: ClassData) => (
  ...
  const handleAddClick = () => navigate('/academics/classes/create-class');
  const handleView = () => navigate('/academics/classes/view-class');
  const handleEdit = () => navigate('/academics/classes/edit-class');
  const handleDelete = (row: ClassData) => { setRowToDelete(row); setIsModalOpen(true); };
